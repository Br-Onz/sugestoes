...
<!-- Navbar-->
<header class="app-header"><a class="app-header__logo" style="font-family: 'Arial Black',serif;font-size: 18px"
                              wire:navigate.hover href="/home"><img src="{{ asset('images/logo.png') }}"
                                                                    style="width: 100px; height: 40px;"></a>
    <!-- Sidebar toggle button--><span x-on:click="open = ! open" style="cursor: pointer" class="app-sidebar__toggle"
                                       href="#" data-toggle="sidebar" aria-label="Hide Sidebar"></span>
    <!-- Navbar Right Menu-->
    <ul class="app-nav">
        <!-- User Menu-->
        <li class="dropdown">
...
<aside class="app-sidebar">
    ...
    <ul class="app-menu">
        @if(!empty(session('pccontro')))
            @foreach(session('pccontro') as $contro)
                @if(isset($contro->pccontroi))
                    @foreach($contro->pccontroi as $Pccontroi)
                        @if($Pccontroi->codrotina == 1444)
                            @if($Pccontroi->codcontrole == 1 && $Pccontroi->acesso == 'S')
                                <li>
                                    <a class="app-menu__item some_no_mobile" href="/home">
                                        <i class="app-menu__icon bi bi-card-text"></i>
                                        <span class="app-menu__label">Sugestão</span>
                                    </a>
                                </li>
                                <li>
                                    <a class="app-menu__item some_no_mobile" href="/solicitados">
                                        <i class="app-menu__icon bi bi-android"></i>
                                        <span class="app-menu__label">Solicitados</span>
                                    </a>
                                </li>
                            @endif
                            @if($Pccontroi->codcontrole == 2 && $Pccontroi->acesso == 'S')
                                <li>
                                    <a class="app-menu__item some_no_mobile" href="/avaliar">
                                        <i class="app-menu__icon bi bi-graph-up-arrow"></i>
                                        <span class="app-menu__label">Avaliar</span>
                                    </a>
                                </li>
                            @endif
                        @endif
